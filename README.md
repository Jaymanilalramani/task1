...
Based on [`order-domain-requirements.json`](https://rabtechacademy.in/starter-files/java/order-domain-requirements.json):
...
| **Rule 5** | *Order total is derived from immutable line prices and quantities.* | Recalculated dynamically as `Σ(line.unitPrice * line.quantity)` upon adding/removing lines. |
...
To compile and execute all 37 tests across the multi-module project:
